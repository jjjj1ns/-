...
if 'chosung1' in st.session_state and 'chosung2' in st.session_state:
    ...
    with col1:
        st.markdown(f"<div style='font-size:60px; text-align:center'>{st.session_state['chosung1']}</div>", unsafe_allow_html=True)
    with col2:
        st.markdown(f"<div style='font-size:60px; text-align:center'>{st.session_state['chosung2']}</div>", unsafe_allow_html=True)
    st.markdown("---")
